...
class StorageManager(private val context: Context) {
    ...
    // Дрібний бонус для дебагу/перевірок
    fun listAllImages(): List<File> {
    ...
    fun deleteImage(photoId: String): Boolean {
        return try {
            val file = getImageFile(photoId)
            if (file.exists()) file.delete() else true
        } catch (e: Exception) {
            Log.e("StorageManager", "❌ Error deleting image $photoId", e)
            false
        }
    }
}
